...
documentation is contained in the JavaDocs (@xref{javadoc,,Creating
JavaDocs}) and in the @emph{Kiss} book, which provides comprehensive
coverage with worked examples.
...
There is normally no need to handle authentication.  @emph{Kiss} handles
that automatically: the caller's session is validated before your method
runs, so an invalid or expired session is rejected for you and your
method is reached only by an authenticated user.

A service may, however, opt out of that automatic check and be invoked
whether or not the caller is logged in -- the ``call the service first,
then let it decide'' model.  Such a service inspects the request (for
example with @code{servlet.isLoggedIn()}) and either returns a public
response or demands a login itself (@code{servlet.requireLogin()}), which
routes the front-end to the login screen exactly as an expired session
would.
...
The two main data types are @code{JSONObject} and @code{JSONArray}.
Common methods include @code{getString()}, @code{getInt()},
@code{getBoolean()}, @code{getDouble()}, @code{getJSONArray()},
@code{has()}, and @code{put()}.  See the JavaDocs for the complete API.

@section Large Language Models

@emph{Kiss} provides a high-level interface to the @emph{Ollama} large language model (LLM), the @emph{OpenAI} (chatGPT) server, the @emph{Anthropic} (Claude) server, and the @emph{OpenRouter} multi-provider gateway. This makes it easy for you to add LLM features to your application.
...
@item
It supports both single-prompt generation (@code{send}) and multi-turn,
role-structured conversations via Ollama's @code{/api/chat} endpoint
(@code{chat}); the latter applies each model's own chat template and exposes
a @code{tools} parameter for function calling.
@item
Both @code{send} and @code{chat} have streaming variants that accept a
callback and deliver the model's reply token-by-token as it is generated,
while also returning the complete assembled text.
@item
...
@emph{OpenAI} serves models through two different endpoints: the
long-standing chat completions endpoint, and the newer responses
endpoint.  Some models are available only on the latter.  By default
this requires no attention: a request goes to chat completions, and if
the provider reports that the model requires the responses endpoint, the
same request is transparently re-sent there and that choice is
remembered for that model.  The method @code{setApi(Api)} pins one
endpoint explicitly, which is useful when pointing the class at a
gateway that implements only one of them.

The static method @code{setUrl(String url)} can be used to override the
default OpenAI chat completions endpoint URL, and
@code{setResponsesUrl(String url)} the responses endpoint URL.  These
are useful for proxies, gateways, or OpenAI-compatible endpoints.  Since
they are global settings, changing them affects all instances.

See that class for documentation.

@subsection Anthropic (Claude)

Like @emph{OpenAI}, the @emph{Anthropic} server runs in the cloud.
In order to use it, you must have an account with Anthropic.
Once you get an account, you will need an API Key to interface with their server.
Once you have this API Key, you may query @emph{Claude}.

The single @emph{KISS} class that interfaces with @emph{Anthropic} is
named @emph{Anthropic}.
...
The static method @code{setUrl(String url)} can be used to override the
default Anthropic endpoint URL.  This is useful for proxies, gateways, or
Anthropic-compatible endpoints.  The static method
@code{setVersion(String version)} can be used to override the
@code{anthropic-version} request header value (e.g., @code{2023-06-01}).
Both are global settings that affect all instances.

See that class for documentation.

@subsection OpenRouter

@emph{OpenRouter} (@uref{https://openrouter.ai}) is a cloud gateway
that provides access to models from many providers (OpenAI, Anthropic,
Google, Meta, Mistral, and others) through a single, OpenAI-compatible
API and a single API key.  In order to use it, you must have an
OpenRouter account and API Key.  Models are identified with a
@code{provider/model} string.

The single @emph{KISS} class that interfaces with @emph{OpenRouter} is
named @emph{OpenRouter}.

Typical usage would look as follows:

@example
    String apiKey = "xxxxxxxxxxxxxxx";
    OpenRouter ai = new OpenRouter(apiKey, "openai/gpt-4o");
    String result = ai.send("Who is president Bush?");
@end example

The class also supports streaming responses and image inputs.
Optional setters cover OpenRouter-specific features: model fallback
routing (@code{setFallbackModels}), unified reasoning-effort control
(@code{setReasoningEffort}), and application attribution headers
(@code{setSiteUrl}, @code{setSiteName}).

The static method @code{setUrl(String url)} can be used to override the
default OpenRouter endpoint URL.  This is useful for proxies or
gateways.  Since it is a global setting, changing the URL affects all
instances.
...
@emph{Kiss} provides a mechanism to execute custom code whenever a user
logs out (manually or due to inactivity timeout).  The handler is
configured in @code{KissInit.groovy} using
@code{UserCache.setLogoutHandler()}, which accepts a
@code{Consumer<UserData>}.
...
@noindent
The @code{UserData} object provides methods such as
@code{getUsername()}, @code{getUserId()}, @code{getUuid()}, and
@code{getUserData(String key)}.  If you need database access in the
handler, open a new connection with
@code{MainServlet.openNewConnection()} and close it in a
@code{finally} block.

To log out from the front-end, call @code{Server.logout()}.

For detailed examples and usage patterns, see the @emph{Kiss} book and
the JavaDocs.
