...
@example
<meta name="kiss-version" content="...">
<meta name="kiss-cache-control" content="false">
@end example
...
resumes.  A production @code{WAR} stamps a fresh version automatically, so
each release force-refreshes all clients with no manual step.

During development the flag is left @code{false}, both so that edited
files are seen immediately on reload and to avoid a needless duplicate
load of @code{index.html}.  The mechanism, and the reason @code{index.html}
