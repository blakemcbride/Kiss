...
@table @code
@item src/main/application/KissInit.groovy
This file is used to configure the system.
@item src/main/application
...
Files under the @code{src/main/webapp} directory represent the front-end of the application.

All files under the @code{src/main/webapp/kiss} directory are part of the @emph{Kiss} system and would normally not need to be touched.

@code{index.html} and @code{index.js} are also part of the @emph{Kiss} system and aren't normally modified.
...
If using @emph{tomcat}, @code{Kiss.war} should be placed in the @code{webapps} directory.  When @emph{tomcat} starts, it will see the file, unpack it, and run it.  The application will be available at @code{[HOST]/Kiss}
