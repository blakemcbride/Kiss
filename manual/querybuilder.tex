...
@deffn {Static Method} SchemaGraph.fromDatabase conn schema
Build or retrieve a cached schema graph for the given schema name.
On the first call for a given schema, this delegates to the
single-argument @code{fromDatabase(conn)} and caches the result.
...
@code{conn} --- a Kiss @code{Connection} object
@item
@code{schema} --- a @code{String} used as the cache key (case-sensitive)
@end itemize

@strong{Returns:} a populated @code{SchemaGraph}, either newly built or from the cache

@strong{Throws:} @code{SQLException} if the graph needs to be built and database metadata cannot be read
@end deffn

@deffn {Static Method} SchemaGraph.clearSchemaCache
Clear the entire internal schema cache used by @code{fromDatabase(conn, schema)}.
After calling this method, subsequent calls will re-read the database metadata.
@end deffn

@deffn {Static Method} SchemaGraph.clearSchemaCache schema
Remove a single entry from the internal schema cache.

@strong{Parameters:}
@itemize @bullet
@item
@code{schema} --- the schema identifier to remove
@end itemize
@end deffn
...
@deffn Method {Connection.newQueryBuilder}
Create a new @code{QueryBuilder} that uses this connection's schema graph
for automatic join resolution.  The connection is stored in the builder
so that the no-argument @code{fetchAll()}, @code{fetchOne()}, and
@code{fetchAllJSON()} methods can be used.  This is the recommended way
to create a @code{QueryBuilder}.

@strong{Returns:} a new @code{QueryBuilder}

@strong{Throws:} @code{SQLException} if the schema graph cannot be obtained
@end deffn

@deffn Constructor QueryBuilder conn
Create a query builder from a @code{Connection}.  Equivalent to
@code{conn.newQueryBuilder()}.
The connection's schema graph is used for join resolution, and the
connection is stored for the no-argument execute methods.
...
@deffn Constructor QueryBuilder graph
Create a query builder with an explicit @code{SchemaGraph}.  The
connection must be passed to the execute methods (@code{fetchAll(conn)},
etc.) when using this constructor.

@strong{Parameters:}
@itemize @bullet
@item
@code{graph} --- the @code{SchemaGraph} describing table relationships
@end itemize
@end deffn
...
@deffn Method fetchAll
@deffnx Method fetchAll conn
Build and execute the query, returning all results as @code{Record}
objects.  The no-argument form uses the stored connection (requires
the builder to have been created with @code{Connection.newQueryBuilder()}
or @code{new QueryBuilder(conn)}).
...
@deffn Method fetchOne
@deffnx Method fetchOne conn
Build and execute the query, returning the first result or @code{null}.
The no-argument form uses the stored connection.
...
@deffn Method fetchAllJSON
@deffnx Method fetchAllJSON conn
...
@deffn Method query
@deffnx Method query conn
Build and execute the query, returning a @code{Cursor} for row-by-row
iteration.  This is useful when a large number of records is possible
and loading all into memory at once is unnecessary.
The no-argument form uses the stored connection.

@strong{Returns:} @code{Cursor}
