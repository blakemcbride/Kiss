...
systems and ``@emph{bld.cmd}'' under Windows.  In this guide,
...
to delete that file prior to IDE configuration.  Deleting it is safe;
the build simply skips it.

Because no build reads that dependency list, a hand-maintained copy of it
would quietly fall behind the list the build actually uses, leaving
@emph{GitHub} reporting against versions the system no longer has.  The
list is therefore generated: everything between the
@code{BEGIN GENERATED DEPENDENCIES} and @code{END GENERATED DEPENDENCIES}
markers is written by the build.  Dependencies are edited in
@code{buildForeignDependencies()} in @code{src/main/precompiled/Tasks.java},
never in @code{pom.xml}.  The rest of @code{pom.xml} is maintained by hand.
...
The @emph{Kiss} documentation consists of three parts; this guide,
...
This guide may be created in two forms.  The first is in an HTML
...
If you are using an alternative data store for user management, you
can force authentication without configuring a SQL database by setting
@code{RequireAuthentication = true} in @code{application.ini}.  When
this option is enabled, the system will delegate authentication to
@code{Login.groovy} even when no SQL database is configured.
...
remainder of the system.  It installs the required dependencies.  It also
deletes superseded versions of those dependencies, so changing a version in
@code{Tasks.java} does not leave the old JAR behind where both would land on
the class path.  JAR files you supplied yourself are never touched.  Finally,
it runs the @code{pom} task described next.
@item pom
This rewrites the dependency list in @code{pom.xml} from the dependencies the
build actually uses, so the two cannot fall out of step.  It normally runs as
part of @code{libs}, and rewrites the file only when its content would
actually change.  See the note about @code{pom.xml} earlier in this chapter.
@item develop
...
the @emph{Kiss} system.  Superseded versions of those files are removed
automatically, so there is nothing to clean up by hand.  JAR files you
added yourself are left alone, and remain your responsibility.
@end itemize
...
For a guided, tutorial-style walkthrough of building applications with
@emph{Kiss} --- from architecture through a complete working
application --- see @emph{Kiss: A Complete Guide to the Web Application
Framework}, available on Amazon.
