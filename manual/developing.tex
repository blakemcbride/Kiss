...
By default, authentication requires a SQL database to be configured.
If no SQL database is configured, the system bypasses authentication
entirely.  However, if your application uses a non-SQL data store
for user management, you can enforce authentication by
setting @code{RequireAuthentication = true} in @code{application.ini}.
When enabled, the system calls @code{Login.groovy} for authentication
regardless of whether a SQL database is configured.  Note that the
@code{db} parameter passed to the @code{login} and @code{checkLogin}
methods will be @code{null} when no SQL database is configured, so
your @code{Login.groovy} must handle this appropriately.

@xref{cached_user_data,,Cached User Data}

@section The Build System

@emph{Kiss} does not use @emph{Ant}, @emph{Maven}, or @emph{Gradle}.
It includes its own build system called @emph{bld}, which is designed
to handle @emph{Kiss}' microservice architecture where the core is
compiled ahead of time but application code is compiled at runtime.

@emph{bld} is made up of two Java files:
@code{src/main/core/org/kissweb/BuildUtils.java} (framework code,
do not modify) and @code{src/main/precompiled/Tasks.java}
(application-specific build procedures).  It can download and cache
dependencies, and only performs work that is needed.  An @emph{Ant}
bridge (@code{build.xml}) is included for IDE integration.

When upgrading @emph{Kiss} (@xref{updates,Kiss Framework Updates}),
@code{Tasks.java} is never overwritten, so you must manually reconcile
any changes.

For a deeper discussion of the build system design and customization,
see the @emph{Kiss} book.
