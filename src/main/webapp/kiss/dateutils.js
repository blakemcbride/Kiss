 ...
 * @returns {*}
 */
dateutils.strToInt = function (dateString) {
 ...
 * @returns {*}
 */
dateutils.SQLtoInt = function (dateString) {
    dateString = dateString.trim();
    dateString = dateString.replace(/-/g, '/');
    dateString = dateString.replace(/\./g, '/');
 ...
 * @returns {boolean}
 */
dateutils.isStrDate = function (val) {
    if (val)
        val = $.trim(val);
    if (!val)
        return false;
 ...
 * @returns {boolean}
 */
dateutils.isSQLDate = function (val) {
    if (val)
        val = $.trim(val);
    if (!val)
        return false;
 ...
 * @param dt
 */
dateutils.intToStr4 = function (dt) {
    if (!dt)
        return '';
 ...
 * @param dt
 */
dateutils.intToSQL = function (dt) {
    if (!dt)
        return '';
    var y = Math.floor(dt / 10000);
    dt -= y * 10000;
 ...
 * @param dt
 */
dateutils.intToStr2 = function (dt) {
    if (!dt)
        return '';
    var y = Math.floor(dt / 10000);
    dt -= y * 10000;
 ...
 * @returns {*}
 */
dateutils.dateToInt = function (dt) {
 ...
 * @constructor
 */
dateutils.intToDate = function (dt) {
    var y = Math.floor(dt / 10000);
    dt -= y * 10000;
    var m = Math.floor(dt / 100);
    var d = Math.floor(dt - m * 100);
...
/**
 * Convert Date object to int date YYYYMMDD
 *
 * @param dt
 * @returns {*}
 * @constructor
 ...
 * @returns {Date}
 */
dateutils.dateAddDays = function (dt, days) {
 ...
 *
 * @param dt int date YYYYMMDD
 * @param days number of days to add or subtract
 * @returns {int}
 */
dateutils.intAddDays = function (dt, days) {
