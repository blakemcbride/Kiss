...
class components {
     ...
     * <br><br>
     * <strong>The open list</strong>
     * <br><br>
     *     When the list is opened with the mouse or the keyboard (a click, <code>Space</code>, <code>Alt+Down</code>,
     *     <code>Alt+Up</code>, or <code>F4</code>), the control draws the list itself, under the control or above it
     *     when there is more room there, rather than using the browser's own pop-up.  The browser delivers every
     *     keystroke to its pop-up and none to the page, so typing to filter (below) could not work there.  The
     *     control's list behaves as the native one does:  the current value is highlighted, <code>Up</code> and
     *     <code>Down</code> move it (firing <code>onChange</code>, as they do in a closed control), a click or
     *     <code>Enter</code> closes the list keeping the value, <code>Escape</code> closes it and returns to the
     *     value it opened with, and clicking elsewhere, leaving the control, or scrolling the page closes it.
     *     Only one drop-down list is ever open at a time.  Touch input is left to the browser's native picker,
     *     which is the better control on a device with no keyboard to filter with.  A read-only drop-down
     *     (<code>readOnly()</code>) neither opens nor changes its value from the keyboard.
     * <br><br>
     * <strong>Typing to filter</strong>
     * <br><br>
     *     While the control has focus, whether or not its list is open, typing narrows the list to the options
     *     whose label contains the typed characters.  Matching is case-insensitive, and the text may appear
     *     anywhere in the label, not only at its start.  With the list open, the typed text is shown at the top
     *     of the list and the rows narrow with each keystroke.  As the user types, the current selection is kept
     *     if it still matches; otherwise the first matching option becomes selected (and <code>onChange</code>
     *     fires, as with the browser's own type-ahead).  <code>Backspace</code> removes the last typed character.
     *     <code>Escape</code> erases the typed sequence, restores the full list, and returns to the selection that
     *     was current before typing began, so the user can start again (an open list stays open; a second
     *     <code>Escape</code> closes it).  Leaving the control also restores the full list, keeping whatever is
     *     then selected.  A lone space opens and closes the list, as it does natively; once a sequence has begun,
     *     a space is part of it.
     * <br><br>
     ...
     *     any method that modifies the list first restores it and closes the list.
     * <br><br>
     * <table>
     *     <tr><th align="left" style="padding-right: 120px;">API</th><th align="left">Description</th></tr>
     ...
     *     its start.  In a single-select list the current selection is kept if it still matches; otherwise the
     *     first matching option becomes selected (and <code>onChange</code> fires, as with the browser's own
     *     type-ahead).  A <code>multiple</code> list only narrows what is shown; every selection is left as it is,
     *     including selections on rows the filter has hidden.  <code>Backspace</code> removes the last typed
     *     character.  <code>Escape</code> erases the typed sequence, restores the full list, and returns to the
     *     selection that was current before typing began, so the user can start again.  Leaving the control also
     *     restores the full list, keeping whatever is then selected.  A leading space keeps its native meaning;
     *     once a sequence has begun, a space is part of it.
     * <br><br>
     *     The filter changes only what the list displays and which options are selected.  <code>size()</code>,
     *     <code>getValue()</code>, <code>getValue(idx)</code>, <code>getLabel(idx)</code>, <code>getAllLabels()</code>,
     *     <code>selectedIndex()</code>, and the other index-based methods always refer to the complete list, and
     *     any method that modifies the list or its selection first restores it.
     * <br><br>
     * <table>
     *     <tr><th align="left" style="padding-right: 120px;">API</th><th align="left">Description</th></tr>
     *     <tr><td>    add(val, lbl, data)     </td><td>     add a new list item.  <code>val</code> is the value associated to the option, <code>lbl</code> is the text shown in the list, and <code>data</data> represents optional and arbitrary data associated to the option               </td></tr>
     ...
     *     <tr><td>    readOnly([flg])     </td><td>     sets control to read-only (or the reverse if the optional argument is <code>false</code>).  A read-only list shows its selection but neither the keyboard nor a click on a row can change it; it can still be scrolled    </td></tr>
     *     <tr><td>    readWrite([flg])     </td><td>     sets control to read-write (or the reverse if the optional argument is <code>false</code>)    </td></tr>
     *     <tr><td>    removeByIndex(idx)     </td><td>     remove the row indicated by <code>idx</code>             </td></tr>
     *     <tr><td>    setLabel(lbl, idx)     </td><td>     sets a row label to <code>lbl</code>.  If <code>idx</code> is undefined, the selected row is affected otherwise the row indexed by <code>idx</code> is updated.             </td></tr>
     *     <tr><td>    setValue(val, idx)     </td><td>     sets a row value to <code>val</code>.  If <code>idx</code> is undefined, the selected row is affected otherwise the row indexed by <code>idx</code> is updated.  </td></tr>
